...
import { Send, Paperclip, MessageSquare, User, Edit2, Check, X } from 'lucide-react';
...
export const ChatView: React.FC<ChatViewProps> = ({ thread, onSendMessage, isLoading, onUpdateTitle }) => {
  ...
  return (
    <div className="h-screen w-full bg-gradient-to-br from-purple-50 to-pink-50 p-4">
      <div className="relative flex flex-col h-full max-w-5xl mx-auto bg-gradient-to-br from-white/75 via-purple-100/60 to-white/75 backdrop-blur-2xl border border-white/60 shadow-[0_20px_60px_-25px_rgba(109,40,217,0.35)] rounded-[26px] overflow-hidden">
      
        {/* Watermark */}    
        {thread.messages.length === 0 && (
          <div className="pointer-events-none select-none absolute inset-0 flex flex-col items-center justify-center opacity-30">
          ...
          {thread.messages.map((msg, index) => (
            <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-lg p-3 rounded-xl ${
                msg.role === 'user' ? 'bg-slate-300 text-gray-900' : 'bg-purple-600 text-white'
              }`}>
                {msg.hasImage && (
        ...
        <div className="p-4 border-t border-purple-600 flex-shrink-0">
          ...
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder="Ask me about your legal case..."
              className="flex-1 h-12 resize-none text-base rounded-full px-6 py-3 placeholder-black/70 border-2 border-gray-300 outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-60"
              rows={1}
            ...
            <button
              ...
              className="h-12 px-6 rounded-full bg-purple-600 hover:bg-purple-700 text-white flex items-center gap-2 disabled:opacity-50 flex-shrink-0"
            >
              Send
