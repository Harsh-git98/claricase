...
export const ChatView: React.FC<ChatViewProps> = ({ thread, onSendMessage, isLoading, onUpdateTitle }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSendMessage(userInput, attachedImage?.base64 || null, isThinkingMode);
    setUserInput('');
      ...
      <div className="p-4 border-b border-slate-200 flex-shrink-0 flex items-center justify-between gap-2 min-h-[65px]">
        {isEditingTitle ? (
            <div className="flex-1 flex items-center gap-2">
                ...
                />
                <button onClick={handleTitleSave} className="p-1 text-green-600 hover:bg-green-100 rounded-full"><CheckIcon className="w-5 h-5"/></button>
                <button onClick={() => { setIsEditingTitle(false); setTitleInput(thread.title); }} className="p-1 text-red-600 hover:bg-red-100 rounded-full"><XIcon className="w-5 h-5"/></button>
            </div>
        ) : (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {thread.messages.map((msg, index) => (
          <div key={index} className={`flex items-start gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            {msg.role === 'assistant' && <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0"><LogoIcon className="w-5 h-5 text-purple-600" /></div>}
            <div className={`max-w-lg p-3 rounded-xl ${msg.role === 'user' ? 'bg-purple-600 text-white' : 'bg-slate-100 text-gray-800'}`}>
              {msg.image && <img src={msg.image} alt="user attachment" className="rounded-md mb-2 max-w-xs" />}
        ...
        {isLoading && (
            <div className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0"><LogoIcon className="w-5 h-5 text-purple-600" /></div>
      ...
      <div className="p-4 border-t border-slate-200 flex-shrink-0">
        ...
        <form onSubmit={handleSubmit} className="flex items-center space-x-2">
          ...
          <textarea
            ...
            disabled={isLoading}
          />
          <button type="submit" className="p-2 rounded-full bg-purple-600 text-white hover:bg-purple-700 disabled:bg-purple-300" disabled={isLoading || (!userInput.trim() && !attachedImage)}>
